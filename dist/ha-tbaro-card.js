...
let HaTbaroCard = class HaTbaroCard extends i {
    setConfig(config) {
        if (!config.entity)
    ...
    render() {
        ...
        const arcs = segments.map(seg => {
            ...
            return b `<path d="${this.describeArc(cx, cy, r, aStart, aEnd)}" stroke="${seg.color}" stroke-width="${stroke_width}" fill="none" />`;
        });
        const ticks = Array.from({ length: 11 }, (_, i) => 950 + i * 10).map(p => {
            ...
            return b `<line x1="${p1.x}" y1="${p1.y}" x2="${p2.x}" y2="${p2.y}" stroke="${tick_color}" stroke-width="2" />`;
        });
        const labels = [960, 980, 1000, 1020, 1040].map(p => {
            const a = Math.PI * 0.75 + ((p - minP) / (maxP - minP)) * Math.PI * 1.5;
            const pt = this.polar(cx, cy, r - 36, a);
            return b `<text x="${pt.x}" y="${pt.y}" font-size="0.9em" font-weight="bolder" class="label">${p}</text>`;
        });
        const needle = (() => {
      ...
      <ha-card style="box-shadow:none;background:transparent;border:none;border-radius:0;">
        ${b `<svg viewBox="0 0 300 300" style="max-width:${size}px;height:auto">
          ${arcs}
          ${ticks}
          ${labels}
          ${needle}
...
__decorate([
    n({ attribute: false })
], HaTbaroCard.prototype, "hass", void 0);
__decorate([
    n()
], HaTbaroCard.prototype, "config", void 0);
HaTbaroCard = __decorate([
    t('ha-tbaro-card')
], HaTbaroCard);

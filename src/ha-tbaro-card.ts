...
interface BaroCardConfig {
  ...
  minor_tick_width?: number;
  minor_tick_length?: number;
}
...
export class HaTbaroCard extends LitElement {
  ...
  render() {
    ...
    const { tick_color, size, segments, angle: gaugeAngle = 270, border = 'outer', stroke_width = 20,
            major_tick_width = 1.5, major_tick_length = 3, minor_tick_width = 1, minor_tick_length = 2 } = this.config;
    ...
    const ticksHpa = [950, 955, 960, 965, 970, 975, 980, 985, 990, 995, 1000, 1005, 1010, 1015, 1020, 1025, 1030, 1035, 1040, 1045, 1050];
    const TICK_LEN_IN = 2;

    const majorTicks = [950, 960, 970, 980, 990, 1000, 1010, 1020, 1030, 1040, 1050].map(p => {
    ...
    const minorTicks = ticksHpa.filter(p => ![950, 960, 970, 980, 990, 1000, 1010, 1020, 1030, 1040, 1050].includes(p)).map(p => {
      const a = startAngle + ((p - minP) / (maxP - minP)) * (endAngle - startAngle);
      const rOuter = r + stroke_width / 2 + minor_tick_length;
      const rInner = r - stroke_width / 2 - TICK_LEN_IN;
      const p1 = this.polar(cx, cy, rOuter, a);
      const p2 = this.polar(cx, cy, rInner, a);
      return svg`<line x1="${p1.x}" y1="${p1.y}" x2="${p2.x}" y2="${p2.y}" stroke="${tick_color}" stroke-width="${minor_tick_width}" />`;
    });
